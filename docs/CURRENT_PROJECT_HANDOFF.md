...
**Phase 6: Mobile 3D gallery controls is now active.** Phase 6A added the first mobile/touch-control baseline for the public Three.js gallery: the gallery no longer routes touch/coarse devices to the desktop-only fallback, it opens the real 3D room, shows a restrained left thumb movement pad, and supports drag-to-look camera movement directly on the canvas. Phase 6B refines that baseline with quieter touch UI, first-use hint fadeout, safer phone/landscape spacing, touch-only analog movement shaping, reduced touch movement speed, and more controlled drag-look sensitivity. Phase 6C cleans up viewer-facing gallery metadata by removing internal wall type labels from plaques and bottom-right artwork info cards. Phase 6D is a narrow local-editor hotfix: the lower Save JSON button on an individual image editor card now builds a card-scoped full-save payload so image metadata edits persist the same way the top Save Changes button does. Phase 6E lightly increased touch movement and drag-look responsiveness after real-phone testing found the controls working but slightly under-sensitive. Phase 6F pulls touch camera sensitivity back toward the Phase 6B baseline after the Phase 6E camera felt slightly too sensitive, while keeping Phase 6E's more responsive movement tuning. Phase 6F also adds a focused homepage landscape-phone layout guard. Phase 6G corrects that landscape-phone homepage guard after real-device testing showed the hero image being visually cut by the copy-panel overlay; the horizontal-phone hero image now fills the available stage and the copy panel is hidden in that mode. Phase 6H broadens and hardens that landscape-phone guard after Pixel 9 Pro XL testing showed the CSS viewport can exceed the old 900px breakpoint, causing the desktop homepage layout to appear in horizontal phone orientation. Phase 6I extends the short-landscape phone treatment to the Portfolio and About routes, and hardens mobile gallery touch interruption handling so active touch movement/look states clear on orientation changes, app switching, window blur, and gallery teardown.
...
The future 3D gallery mobile goal is now active. Phase 6A implements the first functional touch-control baseline: coarse/touch screens can enter the real Three.js gallery, use a left thumb movement pad, and drag the canvas to look around. Phase 6B polishes the touch layer for dev-branch real-device testing: the hint fades after first use, the controls are quieter and more safe-area-aware, touch movement is shaped with a dead zone/curve, and drag-look sensitivity is more restrained. Phase 6C removes internal wall type labels from the public plaque/info-card metadata while preserving wall type data for layout and editor use. Phase 6D fixes the individual image editor's lower Save JSON button so it saves the open image card's edited metadata reliably instead of depending on the broad page-level collector path. Phase 6E lightly increased touch movement speed, analog thumb response, and drag-look sensitivity after real-phone testing confirmed the controls work but could feel slightly more responsive. Phase 6F keeps the Phase 6E movement response but sets touch camera sensitivity between the Phase 6B and Phase 6E values. It also adds horizontal-phone homepage CSS so the hero uses a compact, image-first landscape layout instead of the normal stacked mobile flow. Phase 6G refines that horizontal-phone homepage mode by removing the copy-panel overlay and letting the hero image fill the available landscape stage, which prevents wide hero photos from appearing boxed off or partially covered. Phase 6H makes the same compact image-first homepage layout apply to wider CSS landscape-phone viewports such as the Pixel 9 Pro XL, where the previous max-width breakpoint did not trigger. Phase 6I adds the matching short-landscape phone public-shell guard for Portfolio/About and clears active mobile gallery touch state during orientation/app-focus interruptions. Real-phone/tablet QA should continue from the dev deployment before Phase 6 closeout.
