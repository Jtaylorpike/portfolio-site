...
- Low and the internal balanced tier share the same warm architectural intensity and orange-amber color treatment. Balanced additionally retains the accepted fixed RectAreaLight pool: visible or nearby artwork receives a wall wash without shadows, while pool size stays constant to avoid shader recompilation during movement. Balanced also improves background texture streaming while keeping Low's 0.95 pixel-ratio cap and 1x environment anisotropy. High retains all 23 artwork lights, up to 1.5 pixel ratio, 8x environment anisotropy, 1024px artwork textures, and spotlight shadows.
...
- The earlier post-input regression recorded Low at 96 calls with `0/15 artworks lit (0 lights)`. Medium's active artwork-light count varies with visibility and proximity, while the pool structure remains fixed. The recorded pre-shadow High baseline remains 96 calls with `15/15 artworks lit (23 lights)`.
