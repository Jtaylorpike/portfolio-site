...
- Low hides track fixtures and uses static architectural lighting. Medium retains the fixtures and eight artwork spotlights but omits the fifteen rectangular wall washes, shadow rendering, and 1024px artwork promotion; it also caps pixel ratio at 1.05 and environment anisotropy at 2x. High retains all 23 artwork lights, up to 1.5 pixel ratio, 8x environment anisotropy, 1024px artwork textures, and spotlight shadows.
...
- Audited quality-tier artwork lighting and removed an unintended eight-artwork cap from the High wall-wash pass. Medium uses eight restrained accent spotlights; High adds one broad wall wash for every displayed artwork. Runtime diagnostics should report `8/15 artworks lit (8 lights)` on Medium and `15/15 artworks lit (23 lights)` on High; only High renders spotlight shadow maps.
...
- The earlier post-input regression recorded Low at 96 calls with `0/15 artworks lit (0 lights)` and Medium at `8/15 artworks lit (8 lights)`. The new Medium retains that light count but changes its resolution, texture, and filtering budgets, so it must be remeasured during final validation. The recorded pre-shadow High baseline remains 96 calls with `15/15 artworks lit (23 lights)`.
