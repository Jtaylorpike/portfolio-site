...
- Low uses the brightest architectural treatment with only a restrained warm cast. The internal balanced tier retains its separate warm fill and additionally uses a fixed five-slot RectAreaLight pool. Artwork outside the camera-oriented region is off except for close work directly beside the camera. A widened forward prelight cone and lateral zone activate close work up to 13 meters just before it enters the visible frame, with 15-meter release bounds; meaningfully rearward artwork remains off. The main visible zone reaches 30 meters with a 34-meter release boundary. Five restores the earlier Medium GPU budget while the fixed pool and hysteresis avoid shader recompilation and rapid light reassignment during movement. Balanced also improves background texture streaming while keeping Low's 0.95 pixel-ratio cap and 1x environment anisotropy. High retains all 23 artwork lights, up to 1.5 pixel ratio, 8x environment anisotropy, 1024px artwork textures, and spotlight shadows.
- Medium's five pooled wall washes are 12% stronger, 18% wider, 8% taller, and slightly warmer than High's individual wash recipe. Pool selection favors feature walls and larger artworks after visibility eligibility, improving perceived coverage without adding lights or changing the 100ms update cadence.
