...
**Phase 6: Mobile 3D gallery controls is now active.** Phase 6A adds the first mobile/touch-control baseline for the public Three.js gallery: the gallery no longer routes touch/coarse devices to the desktop-only fallback, it opens the real 3D room, shows a restrained left thumb movement pad, and supports drag-to-look camera movement directly on the canvas.
