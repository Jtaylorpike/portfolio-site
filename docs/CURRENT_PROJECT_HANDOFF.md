...
- Low hides track fixtures and uses static architectural lighting. Medium retains High's fixtures and full per-artwork spotlight/wall-wash look only for up to five artworks inside a margin-expanded camera view zone; the selection refreshes at 10 Hz so off-view lights leave the render set. Medium otherwise uses Low's 0.95 pixel-ratio cap, 520px preview textures, 1x environment anisotropy, and no shadows. High retains all 23 artwork lights, up to 1.5 pixel ratio, 8x environment anisotropy, 1024px artwork textures, and spotlight shadows.
...
- Audited quality-tier artwork lighting and removed an unintended eight-artwork cap from the High wall-wash pass. Medium now procedurally enables the same lights used by High for up to five artworks near the current view while High keeps all fifteen wall washes and eight spotlights active. Medium diagnostics therefore vary with the camera; High should report `15/15 artworks lit (23 lights)` and is the only tier that renders spotlight shadow maps.
