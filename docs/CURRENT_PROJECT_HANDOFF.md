...
- Low and the internal balanced tier share the same static warm architectural lighting and have no per-artwork lights or shadows. Balanced may retain non-emitting track fixtures and improves background texture streaming while keeping Low's 0.95 pixel-ratio cap and 1x environment anisotropy. High retains all 23 artwork lights, up to 1.5 pixel ratio, 8x environment anisotropy, 1024px artwork textures, and spotlight shadows.
...
- Audited quality-tier artwork lighting and removed an unintended eight-artwork cap from the High wall-wash pass. High reports `15/15 artworks lit (23 lights)` and remains the only tier that renders artwork wall washes, spotlights, and shadow maps.
- User direction on 2026-08-06 makes the internal balanced tier visually match Low: both use the same warm architectural intensity and color with no per-artwork lights. Balanced remains a background texture-streaming upgrade that Auto may enter without changing the room's lighting treatment. This supersedes the earlier pooled Medium-lighting experiment.
...
- The earlier post-input regression recorded Low at 96 calls with `0/15 artworks lit (0 lights)`. Balanced now deliberately shares that zero-artwork-light behavior. The recorded pre-shadow High baseline remains 96 calls with `15/15 artworks lit (23 lights)`.
...
1. Complete the final gallery runtime pass across Low, Medium, High, and Auto. Confirm Auto promotion remains stable, Low and the internal balanced tier retain matching lighting/color, High shadows and all-artwork lighting remain correct, and no tier regresses camera movement.
...
- Auto promotion stages the balanced tier's texture readiness and renderer settings without changing Low's lighting/color treatment. High spotlight shadows remain exclusive to explicit High.
