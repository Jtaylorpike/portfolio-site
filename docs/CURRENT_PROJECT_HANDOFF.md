...
- Recalibrated true Low with a 165% architectural-fill lift and a moderate amber-warm balance. The internal balanced tier retains its recovered 75% warm fill and artwork-light pool; High alone uses the accepted neutral architectural-light baseline.
...
- Low uses the brightest architectural treatment with only a restrained warm cast. The internal balanced tier retains its separate warm fill and additionally uses a fixed seven-slot RectAreaLight pool. Artwork outside the camera-oriented region is off except for close work directly beside the camera. A widened forward prelight cone and lateral zone activate close work up to 13 meters just before it enters the visible frame, with 15-meter release bounds; meaningfully rearward artwork remains off. The main visible zone reaches 30 meters with a 34-meter release boundary. The fixed pool and hysteresis avoid shader recompilation and rapid light reassignment during movement. Balanced also improves background texture streaming while keeping Low's 0.95 pixel-ratio cap and 1x environment anisotropy. High retains all 23 artwork lights, up to 1.5 pixel ratio, 8x environment anisotropy, 1024px artwork textures, and spotlight shadows.
- Medium's seven pooled wall washes use a consistent 1.95 luminance across feature, wide, standard, compact, and narrow wall types instead of inheriting High's wall-type intensity curve. They remain 18% wider, 8% taller, and slightly warmer than High's individual wash recipe. Pool selection favors feature walls and larger artworks after visibility eligibility, improving perceived coverage without changing the 100ms update cadence.
...
- Audited quality-tier artwork lighting and removed an unintended eight-artwork cap from the High wall-wash pass. Medium uses a stable five-light pool to reproduce High's rectangular wash on the five highest-priority visible-or-nearby artworks without changing shader light counts while moving. High reports `15/15 artworks lit (23 lights)` and remains the only tier that renders spotlights and shadow maps.
- User direction on 2026-08-06 makes the internal balanced tier's architectural fill and color match Low while preserving Medium's accepted pooled artwork lighting. Removing the pool was an incorrect interpretation and was reversed on 2026-08-07.
...
- Removed the obsolete center-ceiling `ceilingAtmosphereLift` point light after visual review exposed its isolated warm hotspot in the middle of the room. Directional room fill, track fixtures, and High all-artwork illumination remain intact.
...
- The earlier post-input regression recorded Low at 96 calls with `0/15 artworks lit (0 lights)`. Medium keeps five fixed light slots and assigns up to five of them by visibility and proximity. The recorded pre-shadow High baseline remains 96 calls with `15/15 artworks lit (23 lights)`.
...
1. Complete the final gallery runtime pass across Low, Medium, High, and Auto. Confirm Auto promotion remains stable, Low and the internal balanced tier retain matching architectural lighting/color, Medium's pooled artwork washes remain correct, High shadows and all-artwork lighting remain correct, and no tier regresses camera movement.
...
- Auto promotion stages the balanced tier's texture readiness, pooled artwork lighting, and renderer settings while retaining Low's architectural lighting/color treatment. High spotlight shadows remain exclusive to explicit High.
