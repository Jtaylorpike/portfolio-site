...
// Calculates the visible width-to-height ratio of the whole hero carousel stage.
export function getHeroShellAspectRatio(image: GalleryImage): number {
  // Fit Entire Image treats the whole carousel stage as the image frame, so the
  // stage follows the actual photo ratio. This keeps a fitted portrait from
  // sitting inside a wide landscape box and keeps a fitted landscape from being
  // visually cropped by a mismatched stage.
  if (getHeroFitMode(image) === 'contain') {
    return getImageAspectRatio(image);
  }

  // Cover mode intentionally crops to the selected composition frame.
  const frameStyle = getResolvedHeroFrameStyle(image);

  if (frameStyle === 'portrait') {
    return PORTRAIT_FRAME_ASPECT_RATIO;
  }

  if (frameStyle === 'square') {
    return SQUARE_FRAME_ASPECT_RATIO;
  }
